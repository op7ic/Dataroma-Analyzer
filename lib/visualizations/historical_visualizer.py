...
class HistoricalVisualizer:
    ...
    def plot_manager_performance(self, df: pd.DataFrame) -> str:
        ...
        for i, (_, manager) in enumerate(managers.iterrows()):
            ax1.barh(
                i,
                manager["last_year"] - manager["first_year"],
                left=manager["first_year"],
                height=0.8,
                alpha=0.7,
                label=str(manager["manager_name"])[:20],
            )
        ...
        if not consistency_data.empty:
            _scatter = ax3.scatter(  # noqa: F841
                consistency_data["years_active"],
                consistency_data["consistency_score"],
                s=200,
                alpha=0.6,
                c=consistency_data.index,
                cmap="viridis",
            )

            for _, row in consistency_data.iterrows():
                ax3.annotate(
                    row["manager_name"].split("-")[0][:10],
        ...
        action_data.plot(kind="bar", stacked=True, ax=ax4, color=["green", "lightgreen", "orange", "red"])
        ax4.set_xticklabels([m[:15] for m in top_managers["manager_name"].head(10)], rotation=45, ha="right")
    ...
    def plot_crisis_comparison(self, df: pd.DataFrame) -> str:
        ...
        plt.suptitle("Crisis Response Analysis: Comparing Market Behaviors", fontsize=16, fontweight="bold")
        plt.tight_layout()
    ...
    def plot_conviction_plays(self, df: pd.DataFrame) -> str:
        ...
        scatter2 = ax2.scatter(
            ...
            alpha=0.6,
            c=top_conviction["current_holders"],
            cmap="RdYlGn",
            edgecolors="black",
            linewidth=0.5,
        )
        ...
        ax3.bar(
            x - width / 2, holder_data["consistent_managers"], width, label="Long-term Holders", color="blue", alpha=0.7
        )
        ax3.bar(x + width / 2, holder_data["current_holders"], width, label="Current Holders", color="green", alpha=0.7)
    ...
    def plot_stock_life_cycles(self, df: pd.DataFrame, quarters_per_year: pd.Series = None) -> str:
        ...
        width = 0.8
        ax2.bar(
            entry_years.index, entry_years.values, width=width, alpha=0.5, label="Total Stock Entries", color="blue"
        )
        ax2.bar(
            currently_held_by_year.index,
            currently_held_by_year.values,
            width=width,
            alpha=0.8,
            label="Still Held Today",
            color="green",
        )
        ...
        for _, stock in top_stocks.iterrows():
            if stock["accumulation_score"] > 70 or stock["accumulation_score"] < -70:
                ax3.annotate(
                    stock["ticker"],
                    (stock["total_buys"] + stock["total_adds"], stock["total_sells"] + stock["total_reduces"]),
                    xytext=(5, 5),
                    textcoords="offset points",
                    fontsize=8,
                    fontweight="bold",
        ...
        _cbar = plt.colorbar(scatter, ax=ax3, label="Accumulation Score")  # noqa: F841
        ax3.grid(True, alpha=0.3)

        ax3.text(
            0.02,
            0.98,
            "Above line = Net Distribution\nBelow line = Net Accumulation",
            transform=ax3.transAxes,
            va="top",
            fontsize=9,
        ...
        _ = ax4.bar(range(len(interesting_stocks)), interesting_stocks["unique_managers"], color="purple", alpha=0.7)

        for i, (_, stock) in enumerate(interesting_stocks.iterrows()):
            height = stock["unique_managers"]
            ax4.text(i, height + 0.5, f"{int(height)}", ha="center", va="bottom", fontsize=9, fontweight="bold")
