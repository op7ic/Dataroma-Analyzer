...
class CurrentVisualizer:
        ...
        try:
            ...
            if "52_week_low_buys" in results or "52_week_high_sells" in results:
                path = self.create_52_week_chart(
                    results.get("52_week_low_buys"), results.get("52_week_high_sells"), time_period
                )
                if path:
                    viz_paths.append(path)
            ...
            if price_dfs:
                ...
                if low_price_stocks:
                    combined_low_price = pd.concat(low_price_stocks, ignore_index=True).drop_duplicates(
                        subset=["ticker"]
                    )
    ...
    def create_hidden_gems_chart(self, df: pd.DataFrame, time_period: str = "Last 3 Quarters") -> str:
        ...
        try:
            ...
            _ = ax1.barh(top_gems["ticker"], top_gems["hidden_gem_score"], color="purple", alpha=0.7)
    ...
    def create_momentum_chart(self, df: pd.DataFrame, time_period: str = "Last 3 Quarters") -> str:
        ...
        try:
            ...
            _ = axes[0].barh(top_buys["ticker"], top_buys["buy_count"], color="green", alpha=0.7)
            ...
            for i, (ticker, count) in enumerate(zip(top_buys["ticker"], top_buys["buy_count"])):
                axes[0].text(
                    count + max_buys * 0.02, i, f"{count}", va="center", ha="left", fontsize=9, fontweight="bold"
                )

            top_momentum = df.nlargest(15, "momentum_score")
            _ = axes[1].barh(top_momentum["ticker"], top_momentum["momentum_score"], color="blue", alpha=0.7)
            ...
            if not filtered_df.empty:
                ...
                if not cheap_momentum.empty:
                    axes[2].scatter(
                        ...
                        cheap_momentum["current_price"],
                        color="gold",
                        s=100,
                        edgecolors="black",
                        linewidth=2,
                        label=f"Cheap Momentum ({len(cheap_momentum)})",
                        alpha=0.9,
                        zorder=5,
                    )
                ...
                if len(filtered_df) < len(df):
                    ...
                    axes[2].text(
                        0.98,
                        0.98,
                        f"Note: {outliers_count} outlier(s) filtered",
                        transform=axes[2].transAxes,
                        ha="right",
                        va="top",
                        fontsize=8,
                ...
                axes[2].hist(df["momentum_score"], bins=20, color="purple", alpha=0.7, edgecolor="black", linewidth=0.5)
    ...
    def create_price_opportunities_chart(
        self, price_dfs: Dict[str, pd.DataFrame], time_period: str = "Last 3 Quarters"
    ) -> str:
        ...
        try:
            ...
            if manager_distribution:
                ...
                bars2 = axes[0].bar(
                    x + width / 2, manager_counts, width, label="Active Managers", color="navy", alpha=0.7
                )

                axes[0].set_xlabel("Price Range", fontweight="bold")
                axes[0].set_ylabel("Count", fontweight="bold")
                axes[0].set_title(
                    "Opportunities by Price Range: Stocks vs Manager Interest", fontsize=12, fontweight="bold"
                )
                axes[0].set_xticks(x)
                ...
                axes[0].text(
                    0.5, 0.5, "No price range data available", transform=axes[0].transAxes, ha="center", va="center"
                )
            ...
            if all_prices:
                ...
                axes[2].axvline(mean_price, color="red", linestyle="--", label=f"Mean: ${mean_price:.2f}", linewidth=2)
                axes[2].axvline(
                    median_price, color="orange", linestyle="--", label=f"Median: ${median_price:.2f}", linewidth=2
                )
                axes[2].legend()
            ...
            if low_price_managers:
                top_managers = sorted(low_price_managers.items(), key=lambda x: x[1], reverse=True)[:10]
                mgr_names = [m[0][:20] for m in top_managers]  # Truncate long names
                mgr_counts = [m[1] for m in top_managers]

                _ = axes[3].barh(mgr_names, mgr_counts, color="darkgreen", alpha=0.7)
    ...
    def create_52_week_chart(
        self, low_buys_df: pd.DataFrame, high_sells_df: pd.DataFrame, time_period: str = "Last 3 Quarters"
    ) -> str:
        ...
        try:
            ...
            if low_buys_df is not None and not low_buys_df.empty:
                ...
                if low_managers:
                    top_low_mgrs = sorted(low_managers.items(), key=lambda x: x[1], reverse=True)[
                        :8
                    ...
                    for i, (name, count) in enumerate(top_low_mgrs):
                        axes[2].text(
                            i, count + 0.1, f"{count}", ha="center", va="bottom", fontsize=9, fontweight="bold"
                        )
    ...
    def create_new_positions_analysis_chart(self, df: pd.DataFrame, time_period: str = "Last 3 Quarters") -> str:
        ...
        try:
            ...
            HIGH_CONVICTION_THRESHOLD = (
                df["portfolio_percent"].quantile(0.9) if "portfolio_percent" in df.columns else 5.0
            )
            ...
            if "current_price" in df.columns and "portfolio_percent" in df.columns:
                ...
                _scatter = axes[0].scatter(  # noqa: F841
                    filtered_df["current_price"],
                    filtered_df["portfolio_percent"],
                    alpha=0.6,
                    s=60,
                    c="steelblue",
                    edgecolors="white",
                ...
                if not high_conviction.empty:
                    axes[0].scatter(
                        high_conviction["current_price"],
                        high_conviction["portfolio_percent"],
                        color="gold",
                        s=100,
                        edgecolors="black",
                        linewidth=2,
                ...
                axes[0].text(
                    0.5,
                    0.5,
                    "Price or portfolio weight data not available",
                    transform=axes[0].transAxes,
                    ha="center",
                    va="center",
                )
            ...
            if "portfolio_percent" in df.columns:
                ...
                _ = axes[1].barh(
                    ...
                    top_positions["portfolio_percent"],
                    color="darkgreen",
                    alpha=0.8,
                    edgecolor="black",
                    linewidth=0.5,
            ...
            return self._complete_new_positions_chart(
                axes, df, time_period, fig, HIGH_CONVICTION_THRESHOLD, LOW_PRICE_THRESHOLD
            )
    ...
    def _complete_new_positions_chart(
        self,
        axes,
        df: pd.DataFrame,
        time_period: str,
        ...
        if "portfolio_percent" in df.columns:
            ...
            axes[2].axvline(
                x=mean_weight, color="red", linestyle="-", alpha=0.8, linewidth=2, label=f"Mean: {mean_weight:.1f}%"
            )
            axes[2].axvline(
                x=median_weight,
                color="orange",
        ...
        if not df.empty:
            ...
            if "portfolio_percent" in df.columns:
                ...
                if high_conviction_count > 0:
                    ...
                    high_conv_tickers = [
                        f"{row['ticker']} ({row['portfolio_percent']:.1f}%)"
                        for _, row in high_conv_top3.iterrows()
                        if pd.notna(row["portfolio_percent"])
                    ]
                    table_data.append(
                        [
            ...
            if "current_price" in df.columns:
                ...
                if low_price_count > 0:
                    ...
                    low_price_tickers = [
                        ...
                        for _, row in low_price_top3.iterrows()
                        if pd.notna(row["current_price"])
                    ]
                    table_data.append(
                        ["Low Price (<$50)", f"{low_price_count} positions", ", ".join(low_price_tickers[:3])]
                    )
                else:
            ...
            if "current_price" in df.columns and "portfolio_percent" in df.columns:
                sweet_spot_df = df[
                    (df["current_price"].fillna(float("inf")) < low_price_threshold)
                ...
                if sweet_spot_count > 0:
                    ...
                    table_data.append(
                        ["Sweet Spot (Low + High)", f"{sweet_spot_count} positions", ", ".join(sweet_spot_tickers)]
                    )
                else:
            ...
            if "value" in df.columns:
                ...
                if len(valid_values) > 0:
                    ...
                    table_data.append(
                        ["Position Values", f"Avg: ${avg_value/1e6:.1f}M", f"Median: ${median_value/1e6:.1f}M"]
                    )
                else:
        ...
        plt.suptitle(
            f"New Positions Analysis: Price vs Portfolio Weight ({time_period})", fontsize=16, fontweight="bold", y=0.95
        )
        ...
        try:
            ...
            if low_price_df.empty:
                ...
                if "manager_count" in low_price_df.columns:
                    ...
                    if manager_col == "managers":
                        ...
                        low_price_df = low_price_df.merge(
                            manager_counts.rename("manager_count"), left_on="ticker", right_index=True, how="left"
                        )
                else:
                ...
                _ = axes[0].barh(top_accumulation["ticker"], top_accumulation[sort_col], color="darkgreen", alpha=0.7)
            ...
            if not low_price_df.empty and manager_col:
                ...
                if manager_col == "managers":
                    ...
                    top_stocks = low_price_df.nlargest(
                        10, "accumulation_score" if "accumulation_score" in low_price_df.columns else "manager_count"
                    )
            ...
            if not low_price_df.empty:
                ...
                table_data.append(
                    ["TOTAL", f"${avg_price:.2f}", f"{unique_managers}", f"{total_positions}", time_period[:12]]
                )

                # Create full-sized table that fills the entire panel
                table = axes[3].table(cellText=table_data, colLabels=headers, loc="center", cellLoc="center")
                table.auto_set_font_size(False)
                table.set_fontsize(10)  # Reasonable font size
            ...
            plt.suptitle(f"Low-Price Stock Accumulation Analysis ({time_period})", fontsize=16, fontweight="bold")
            plt.tight_layout()
    ...
    def create_portfolio_changes_chart(self, df: pd.DataFrame, time_period: str = "Last 3 Quarters") -> str:
        ...
        try:
            ...
            if score_col in df.columns:
                ...
                axes[2].axvline(mean_score, color="red", linestyle="--", label=f"Mean: {mean_score:.1f}", linewidth=2)
                axes[2].legend()
            ...
            plt.suptitle(f"Portfolio Concentration Changes ({time_period})", fontsize=16, fontweight="bold")
            plt.tight_layout()
    ...
    def _create_empty_chart(self, message: str) -> str:
        ...
        ax.text(0.5, 0.5, message, transform=ax.transAxes, ha="center", va="center", fontsize=16, fontweight="bold")
        ax.axis("off")
