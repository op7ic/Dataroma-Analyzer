...
class GemsAnalyzer(MultiAnalyzer):
    ...
    def _fix_grouped_columns(self, df: pd.DataFrame, expected_columns: List[str]) -> pd.DataFrame:
        ...
        logging.warning(
            "Column length mismatch: expected %d, got %d. Expected: %s, Actual: %s",
    ...
    def analyze_hidden_gems(self) -> pd.DataFrame:
        ...
        if "current_price" in holdings.columns and "reported_price" in holdings.columns:
            price_data = holdings.groupby("ticker").agg({"current_price": "first", "reported_price": "first"})

            # Add 52-week data if available
            if "52_week_low" in holdings.columns and "52_week_high" in holdings.columns:
                week_52_data = holdings.groupby("ticker").agg({"52_week_low": "first", "52_week_high": "first"})
        ...
        hidden_gems.loc[(hidden_gems["manager_count"] == 1) & (hidden_gems["max_portfolio_pct"] > 5), "gem_type"] = (
            "Conviction Play"
        )
        hidden_gems.loc[(hidden_gems["price_momentum_score"] > 0.7), "gem_type"] = "Value Opportunity"
        hidden_gems.loc[
            (hidden_gems["recent_activity_score"] > 0.5) & (hidden_gems["manager_count"] <= 2), "gem_type"
        ] = "Emerging Pick"
        ...
        if self.data.history_df is not None and not self.data.history_df.empty:
            recent_tickers = set(
                self.data.history_df[self.data.history_df["period"].isin(self.get_recent_quarters(3))][
                    "ticker"
                ].unique()
            )
        ...
        result = self.format_output(hidden_gems.reset_index()).head(50)
        return self.add_metadata_columns(result, window_quarters=3, analysis_type="hidden_gems")
    ...
    def analyze_deep_value_plays(self) -> pd.DataFrame:
        ...
        """
        if not self.validate_required_columns(self.data.holdings_df, ["ticker", "current_price", "reported_price"]):
            return pd.DataFrame()
        ...
        price_analysis = holdings.groupby("ticker").agg(agg_dict)
        ...
        price_analysis["price_change_pct"] = (
            ...
            / price_analysis["avg_reported_price"]
            * 100
        )
        ...
        if "52_week_low" in holdings.columns and "52_week_high" in holdings.columns:
            week_52_data = (
                holdings.groupby("ticker").agg({"52_week_low": "first", "52_week_high": "first"}).reset_index()
            )
            price_analysis = price_analysis.merge(week_52_data, on="ticker", how="left")
            ...
            # Identify stocks near 52-week lows (bottom 25% of range)
            price_analysis["near_52w_low"] = price_analysis.apply(
                lambda row: self.calc.is_near_52_week_low(
                    row["current_price"], row["52_week_low"], row["52_week_high"], 25.0
                ),
                axis=1,
            )
        else:
        ...
        value_plays = price_analysis[
            (
                (price_analysis["price_change_pct"] < -2.0)
                | price_analysis["near_52w_low"].eq(True)  # Down 2%+ from reported
                | (price_analysis["52_week_position_pct"] < 30)  # Near 52w low  # In bottom 30% of range
            )
        ...
        value_plays["value_score"] += value_plays["price_change_pct"].clip(-50, 5).apply(lambda x: max(0, -x)) / 10

        # Factor 2: 52-week position (lower position = higher score)
        value_plays["value_score"] += (100 - value_plays["52_week_position_pct"]) / 20

        # Factor 3: Manager conviction (higher allocation = higher score)
        value_plays["value_score"] += value_plays["avg_portfolio_pct"] / 5

        # Factor 4: Manager count (more managers = more conviction)
        value_plays["value_score"] += value_plays["manager_count"] / 10

        # CRITICAL: Filter to only stocks with recent activity for "current" classification
        if (
            self.data.history_df is not None
            and not self.data.history_df.empty
            and "action_type" in self.data.history_df.columns
            ...
            recent_buys = (
                ...
                .groupby("ticker")
                .size()
                .to_frame("recent_buys")
            )
            value_plays = value_plays.merge(recent_buys, on="ticker", how="left")
            value_plays["recent_buys"] = value_plays["recent_buys"].fillna(0)
            value_plays = value_plays[value_plays["recent_buys"] > 0].copy()
            logging.info(f"Filtered deep_value_plays to {len(value_plays)} stocks with recent activity")

            if value_plays.empty:
                logging.warning("No deep value plays with recent activity")
                return pd.DataFrame()

        # Categorize value type
        value_plays["value_type"] = "Value Play"
        value_plays.loc[value_plays["near_52w_low"].eq(True), "value_type"] = "52-Week Low Value"
        value_plays.loc[value_plays["price_change_pct"] < -10, "value_type"] = "Deep Discount"

        # Sort by value score
        value_plays = value_plays.sort_values(by="value_score", ascending=False)

        result = self.format_output(value_plays.reset_index()).head(30)
        return self.add_metadata_columns(result, window_quarters=3, analysis_type="deep_value_plays")
    ...
    def analyze_contrarian_opportunities(self) -> pd.DataFrame:
        ...
        if (
            ...
            or "action_type" not in self.data.history_df.columns
        ):
            logging.warning("No activity data available for contrarian analysis")
            return pd.DataFrame()

        # Get recent activities (last 3 quarters)
        recent_quarters = self.get_recent_quarters(3)
        recent_activities = self.data.history_df[self.data.history_df["period"].isin(recent_quarters)].copy()
        ...
        # Extract buy and sell counts from dict before converting
        ...
        # Replace action_breakdown dict with readable string format
        activity_summary["action_breakdown"] = activity_summary["action_breakdown"].apply(
            lambda x: ", ".join([f"{action}: {count}" for action, count in sorted(x.items())])
        )
        ...
        contrarian_stocks["contrarian_score"] = (
            ...
            * contrarian_stocks["active_managers"]
            / 10
        )
        ...
        contrarian_stocks["contrarian_signal"] = contrarian_stocks.apply(
            lambda row: (
                ...
                else "Net Selling" if row["sell_count"] > row["buy_count"] else "Mixed Signal"
            ),
            axis=1,
        )
        ...
        # Add manager details
        buying_managers = (
            recent_activities[recent_activities["action_type"].isin(["Buy", "Add"])]
        ...
        selling_managers = (
            recent_activities[recent_activities["action_type"].isin(["Sell", "Reduce"])]
            .groupby("ticker")["manager_id"]
            .apply(lambda x: ", ".join(x.unique()))
        ...
        result = self.format_output(contrarian_stocks.reset_index()).head(30)
        return self.add_metadata_columns(result, window_quarters=3, analysis_type="contrarian_opportunities")
    ...
    def analyze_under_radar_picks(self) -> pd.DataFrame:
        ...
        if "portfolio_percent" in holdings.columns:
            ...
            under_radar.columns = ["manager_count", "managers", "avg_portfolio_pct", "total_value", "total_shares"]
        ...
        under_radar = under_radar[
            (under_radar["manager_count"] <= 2) & (under_radar["max_portfolio_pct"] > 3.0)  # At least 3% position
        ].copy()
        ...
        if (
            ...
            # Check for recent activity (Buy or Add)
            recent_activity = (
                self.data.history_df[
                    (self.data.history_df["action_type"].isin(["Buy", "Add"]))
                    & (self.data.history_df["period"].isin(recent_quarters))
            ...
            premium_picks = premium_picks.join(recent_activity, how="left")
            premium_picks["recent_additions"] = premium_picks["recent_additions"].fillna(0)

            # CRITICAL: Filter to only stocks with recent activity for "current" classification
            premium_picks = premium_picks[premium_picks["recent_additions"] > 0].copy()
            logging.info(f"Filtered under_radar_picks to {len(premium_picks)} stocks with recent activity")

            if premium_picks.empty:
                logging.warning("No under-radar picks with recent activity")
                return pd.DataFrame()
        ...
        premium_picks.loc[premium_picks["manager_count"] == 1, "pick_type"] = "Exclusive Pick"
        premium_picks.loc[premium_picks["recent_additions"] > 0, "pick_type"] = "Growing Interest"
        premium_picks.loc[premium_picks["manager_quality"] >= 1.8, "pick_type"] = "Premium Pick"

        # Sort by under-radar score
        premium_picks = premium_picks.sort_values(by="under_radar_score", ascending=False)

        result = self.format_output(premium_picks.reset_index()).head(40)
        return self.add_metadata_columns(result, window_quarters=3, analysis_type="under_radar_picks")
